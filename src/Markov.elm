...
import List
import Models exposing (MarkovConfig, MarkovDict, Prediction(..), SingleMarkovConfig, TokenizationType(..))
...
buildMarkov sourceText config =
    let
        tokens =
            tokenize sourceText config.tokenizationType
    in
...
addOneToKey key maybeDict =
    case maybeDict of
        Just dict ->
            Just (Dict.update key addOne dict)

        Nothing ->
            Just (Dict.insert key 1 Dict.empty)
